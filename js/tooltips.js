...
// Definir una función para agregar tooltips a los nucleos
function agregarTooltipEstados(feature, layer) {
    // Crear la función para mostrar u ocultar el tooltip según el nivel de zoom
    function toggleTooltip() {
        if (map.getZoom() >= 12) {
            if (!layer.getTooltip()) {
               layer.bindTooltip(feature.properties.NOM_ENT, {
                    permanent: true,
                    direction: 'center',
                    className: 'noBorderTooltipEstados'
                }).openTooltip();
            }
        } else {
            if (layer.getTooltip()) {
    ...
    layer.on("mouseover", function(e) 
    {
        ...
        // Contenido del tooltip temporal
        var tooltipContent = "<b>Estado: </b>" + feature.properties.NOM_ENT +
            "<br><b>Habitantes: </b>" + feature.properties.POB_TOT +
            "<br><b>Superficie (Has): </b>" + feature.properties.SUP_TOT;
    ...
    layer.on("mouseout", function(e) 
    {
        ...
        var originalStyle = cargarStyleEnt(feature);  // Obtener el estilo original del polígono
        layer.setStyle(originalStyle);  // Restaurar el estilo original
        ...
        // Reagregar el tooltip original
        layer.unbindTooltip();
        if (map.getZoom() >= 12) {layer.bindTooltip(feature.properties.NOM_ENT, 
        {
            permanent: true,  // El tooltip es permanente
            direction: 'center',  // Dirección del tooltip
            className: 'noBorderTooltipEstados'  // Clase CSS para el tooltip
        }).openTooltip();
    }
    });
}